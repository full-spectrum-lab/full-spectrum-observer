...
if ($Gate -eq "IG6") {
    ...
    foreach ($FormalGate in @("IG3", "IG4", "IG5")) {
        ...
        $ScriptName = switch ($FormalGate) {
            "IG3" { "ig3-reference-validator.py" }
            "IG4" { "ig4-worker-smoke.py" }
            "IG5" { "ig5-reference-pipeline.py" }
        }
        & $PrivatePython (Join-Path $PSScriptRoot $ScriptName)
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }

    $HarnessArgs = @((Join-Path $PSScriptRoot "ig6-harness.py"))
    if ($GenerateEvidence) { $HarnessArgs += "--generate-evidence" }
    & $PrivatePython @HarnessArgs
    exit $LASTEXITCODE
}
