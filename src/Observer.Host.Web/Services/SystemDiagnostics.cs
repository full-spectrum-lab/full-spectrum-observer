...
public sealed class SystemDiagnostics
{
    ...
    private static ExternalReleaseIdentity? LoadExternalIdentity(string path)
    {
        try
        {
            ...
            {
                ObserverVersion = ReadString(root, "observer_version", "product_version", "version"),
                ObserverCommit = ReadString(root, "observer_commit", "source_commit", "commit"),
                PackageSha256 = ReadString(root, "package_sha256"),
                BuildChannel = ReadString(root, "build_channel", "channel"),
                ReleaseStatus = ReadString(root, "release_status", "status"),
            };
        }
        catch
        {
